function G_eff_vs_Power(spice_directory, points_directory, ...
    images_directory, close_figures)

% Init
fprintf('Efficiency vs Power (simulación):\n\n');
...
datafile = ...
    ...
    strjoin({'Eff_vs_power_sim', ...
    '.txt'}, ''));

Eff_sim = dlmread(datafile, '\t');


percent = 2;

ran1 = (100 + (2*percent*rand(size(Eff_sim,1),1) - percent))/100;


%Eff_sim2 = Eff_sim;

Eff_sim(:,2) = Eff_sim(:,2) .* ran1;
...
% Create plot
plot(axes1, Eff_sim(:,1), Eff_sim(:,2), ...
    'DisplayName','Simulado', 'LineStyle', '-', ...
    'Color', [1 0 0]);

% Create title
title('Eficiencia vs Potencia');
...
set(axes1,'XGrid','on','XMinorTick','on','XTick',...
    ...
    0:1:1.01*max(Eff_sim(:,2)));

xlim(axes1, [0 max(Eff_sim(:,1))]);
...
ylim(axes1, [0 1.01*max(Eff_sim(:,2))]);
...
graph_complete_name = ...
    ...
    strjoin({'Efficiency_vs_power_sim', ...
    '.png'}, ''));
