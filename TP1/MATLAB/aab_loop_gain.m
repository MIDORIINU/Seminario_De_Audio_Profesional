...
% Create plot
semilogx(freq, ...
    mag2db(mod_data), 'Parent', subplot_module, 'Tag', 'Mod');
...
% Create plot
semilogx(freq, ...
    rad2deg(angle_data), 'Parent', subplot_phase, 'Tag', 'Phase');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Agrego markers customizados.


plot(subplot_module, freq(GM_index), mag2db(mod_data(GM_index)), ...
    'o', 'Color', fig_colors(1, :));


plot(subplot_phase, freq(PM_index), rad2deg(angle_data(PM_index)), ...
    'o', 'Color', fig_colors(2, :));
...
function output_txt = customDatatipFunction1(~,evt, freq, module, angle)

target = get(evt, 'Target');
idx = get(evt, 'DataIndex');
pos = get(evt, 'Position');
%
tag = get(evt.Target, 'Tag');
...
if (~isempty(idx_f) && ~isempty(special_index) && (special_index == idx))
...
function output = freqscale(freq)

if (freq < 0.1)
    output = sprintf('%.3f mHz', freq*1000);
    return;
elseif ((freq >= 0.1) && (freq < 1E3))
    output = sprintf('%.3f Hz', freq);
    return;
elseif ((freq >= 1E3) && (freq < 1E6))
    output = sprintf('%.3f kHz', freq/1E3);
    return;
elseif ((freq >= 1E6) && (freq < 1E9))
    output = sprintf('%.3f MHz', freq/1E6);
    return;
else
    output = sprintf('%.3f GHz', freq/1E9);
    return;
end
