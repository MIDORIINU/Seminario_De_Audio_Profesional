function G_thd_vs_Freq_sim(spice_directory, points_directory, ...
    images_directory, close_figures)

% Init
fprintf('THD vs Frequency (simulación):\n\n');
...
set(axes1,'XScale','log','XGrid','on','XMinorTick','on','XTick',...
    [min(THD_sim(:,1)):20:300 400:100:900 ...
    1000:1000:max(THD_sim(:,1))],...
    'YGrid','on','YMinorTick','on','YTick',...
    0:0.025:1.1*max(THD_sim(:,2)));

xlim(axes1, [min(THD_sim(:,1)) max(THD_sim(:,1))]);
...
ylim(axes1, [0 1.1*max(THD_sim(:,2))]);
