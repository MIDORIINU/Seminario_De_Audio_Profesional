function [figure_handle] = aab_dynamic_response(file_name, ...
    ...
    vout_limits, vout_ticks, vout_exp, slew_rate_limits)
...
if ((slew_rate_limits(1) ~= 0) || (slew_rate_limits(2) ~= 0))
    
    [~, idx1] = min(abs(time - slew_rate_limits(1)));
    
     [~, idx2] = min(abs(time - slew_rate_limits(2)));   
    
    m = 1E-6*(Vout(idx2) - Vout(idx1))/ ...
        (time(idx2) - time(idx1)) ;
    
    annotation(figure_handle,'textbox',...
        [0.44 0.304 0.11 0.040],...
        'Color',fig_color_order(2, :),...
        'String',{sprintf('Slew Rate: %.2f $$\\frac{v}{\\mu s}$$', m)},...
        'LineStyle',':',...
        'FontWeight','bold',...
        'FitBoxToText','on',...
        'EdgeColor',fig_color_order(2, :), ...
        'Interpreter', 'latex');
