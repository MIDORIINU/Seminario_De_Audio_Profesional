...
plot2 = semilogx(freq, rad2deg(unwrap(angle(complex_impedance))), ...
...
output_txt = {sprintf(strjoin({'%s\nFrec:      %s\n|Z|:', ...
    '         %s\n ang(Z): %.2f º'}, ''), ...
    customtitles{idx_f}, get_frequency_in_correct_unit(X1(idx)), ...
...
function val = get_frequency_in_correct_unit(res)

if (res >= 1E3)
    val = sprintf('%.2f kHz', res / 1E3);
elseif (res >= 1E6)
    val = sprintf('%.2f MHz', res / 1E6);
else
    val = sprintf('%.2f Hz', res);
end
